...
import { Rocket, Users, Globe, Award, Heart, ShieldCheck, ArrowRight, Zap, Palette } from 'lucide-react'
...
export default function AboutPage() {
  ...
  const partners = [
    { name: 'StreamTech', Icon: Rocket, color: 'text-blue-500' },
    { name: 'Nova Art', Icon: Palette, color: 'text-purple-500' },
    { name: 'V-Unity', Icon: Globe, color: 'text-emerald-500' },
    { name: 'CyberCore', Icon: Zap, color: 'text-amber-500' }
  ]
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="mb-48 relative">
            ...
            <div className="relative">
              {/* Vertical Line */}
              <div className="absolute left-0 md:left-1/2 top-0 bottom-0 w-px bg-black dark:bg-white" />
              ...
              <div className="space-y-32">
                {journey.map((item, i) => (
                  ...
                  >
                    ...
                    <div className={`w-full md:w-1/2 pl-10 md:pl-0 ${i % 2 === 0 ? 'md:pl-24 md:pr-16' : 'md:pr-24 md:pl-16'}`}>
                      <div className={`${i % 2 === 0 ? 'md:text-left' : 'md:text-right'}`}>
                        <span className="block text-4xl font-black text-[#007AFF] mb-4">{item.year}</span>
                        <h4 className="text-2xl font-bold text-[#1D1D1F] dark:text-[#F5F5F7] mb-4 tracking-tight">{item.title}</h4>
                        <p className="text-lg text-[#1D1D1F]/50 dark:text-slate-400 font-medium leading-relaxed">{item.desc}</p>
                      </div>
                    </div>
                ...
                <div className="space-y-12">
                  <div className="flex items-start gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-[#1D1D1F]/50 dark:text-slate-400 font-medium text-lg leading-relaxed">Every stroke and every line of code is crafted to perfection, ensuring your model stands out.</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-[#1D1D1F]/50 dark:text-slate-400 font-medium text-lg leading-relaxed">We listen to your vision. Your model is a reflection of your personality, and we honor that.</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-12">
                {[
                  ...
                  <div key={i} className="text-center md:text-left">
                    ...
                    <span className="text-xs font-black uppercase tracking-[0.3em] text-[#1D1D1F]/30 dark:text-slate-500">{item.label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Partners Section (Minimalist Logos) */}
          <div className="mb-16">
            <h2 className="text-xs font-black text-center text-[#1D1D1F]/20 dark:text-slate-600 uppercase tracking-[0.4em] mb-24">Strategic Collaborations</h2>
            <div className="flex flex-wrap justify-center items-center gap-x-24 gap-y-16">
              {partners.map((partner) => (
                <div key={partner.name} className="group relative flex flex-col items-center">
                  <div className="relative mb-4 grayscale opacity-30 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-700">
                    <div className="flex items-center space-x-3">
                      <partner.Icon className={`${partner.color} transition-transform group-hover:scale-110`} size={32} strokeWidth={1.5} />
                      <span className="text-2xl font-black text-[#1D1D1F] dark:text-[#F5F5F7] tracking-tighter">{partner.name}</span>
                    </div>
                  </div>
                  <div className="h-[2px] w-0 bg-[#007AFF] group-hover:w-full transition-all duration-500" />
                </div>
              ))}
            </div>
          </div>
